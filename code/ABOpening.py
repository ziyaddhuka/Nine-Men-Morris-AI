...
class ABOpening:
    ...
    def a_b_MaxMin(self, board, depth, alpha, beta):
        if depth == 0:
            self.count_pos += 1
          # print("Max returns ", static_estimation_opening(board))
            ...
            init_board = None
            for a_board in L:
                v_temp,new_board = self.a_b_MinMax(a_board, depth-1, alpha, beta)
                if v_temp > v:
                    v = v_temp
    ...
    def a_b_MinMax(self, board, depth, alpha, beta):
        if depth == 0:
            self.count_pos += 1
          # print("Min Returns", static_estimation_opening(board))
            ...
            init_board = None
            for a_board in L:
                v_temp, new_board = self.a_b_MaxMin(flip_board(a_board), depth-1, alpha, beta)
                if v_temp < v:
                    v = v_temp
                    init_board = a_board.copy()
                if v <= alpha:
...
if __name__=='__main__':
    ...
    board = list(board)
    game = ABOpening()
    static, next_move = game.a_b_MaxMin(board, depth, -10**299, 10**299)
