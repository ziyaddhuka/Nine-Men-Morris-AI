...
class MiniMaxOpeningImproved:
    ...
    def new_MaxMin(self, board, depth):
        if depth == 0:
            ...
            init_board = None
            for a_board in L:
                v_temp,new_board = self.new_MinMax(a_board, depth-1)
                if v_temp > v:
                    v = v_temp
    ...
    def new_MinMax(self, board, depth):
        if depth == 0:
            self.count_pos = self.count_pos + 1
            return improved_static(board), board
        else:
            ...
            init_board = None
            for a_board in L:
                v_temp, new_board = self.new_MaxMin(flip_board(a_board), depth-1)
                if v_temp < v:
                    v = v_temp 
                    init_board = a_board.copy()
...
if __name__=='__main__':
    ...
    static, next_move = game.new_MaxMin(board, depth)
